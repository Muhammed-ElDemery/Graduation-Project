...
NF_dB_Amp = 3;
AMP_Range = 10+5;
%OIP3_Amp_vector = [ 26.210   26.027   25.807   25.545   25.236   24.876   24.461   23.990   23.461   22.876   22.236   21.545   20.807   20.027   19.210 18.361];

%OIP3_Amp_vector =   [26.530   26.420   26.230   25.940   25.560   25.090   24.540   23.930   23.160   22.410   21.390   20.490   19.570   18.770   17.980 17.090];

OIP3_Amp_vector = 25*ones(1,16);
NF_Amp_vector = 3*ones(1,16);
%% -------------------- Derived Parameters --------------------
...
%    w = blackmanharris(N); WinGain = sum(w)/N;
    ...
    xlabel('Frequency (MHz)'); ylabel('Output Power (dBm)');
    title('Output Spectrum (with LNA Nonlinearity)');
    xlim([0 200e6/1e6]);
end
...
if test_type == 5
    ...
    [Z , P , G ] = ellipap(5,0.5,30);
    filter_tf = zpk(Z*250e6*2*pi,P*250e6*2*pi,G*250e6*2*pi);
    [MAG PHA W] = bode(filter_tf,2*pi*f);
   ...
   % f2 = input('Enter desired tone 2 (Hz): ');
	...
	f2 = 1.18e9; %FAST_OP
    f_LO=1e9;
    BW_chan = 400e6; % channel bandwidth
...
%(Power_befor_LPF_vec(k-1) >= LPF.OP1dB - G_dB_LPF - 6 )
			...
			if(round(Pout_ADC_vec2(k-1)) >= 4)
				NUMBER_OF_STEPS = ceil(Pout_ADC_vec2(k-1)  - 4 )+1-NEGATIVE_STEPS;
        ...
        OIP3_Amp_now = OIP3_Amp_vector(G_dB_AMP_MAX - G_dB_Amp+1);
        NF_Amp_now = NF_Amp_vector(G_dB_AMP_MAX - G_dB_Amp+1);
		...
		AMP = analyze_block(G_dB_Amp, OIP3_Amp_now, OIP2_dB_Amp, NF_Amp_now, Z0, B, T,MODEL_IP2);
        % ---------- LNA output ----------
        % Pass through nonlinear LNA model
        V_LO = 2*cos(2*pi*f_LO*t);
        % Nonlinear output + noise
...
%        Pin_W   = (abs(Vin_fft)).^2 / Z0;
		...
		vnoise_rms = sqrt(No * BW_chan * G_lin * (10^(NF_total/10)));
    ...
    ylabel('dB');
    title(['SNR, SDR, SNDR vs Input Power , Pin min = ', num2str(Pin_Min(1))]);
	...
	figure;
	Pin_Max = zerocrossing(Pin_dBm_vec+3,Pout_ADC_vec-14);
	Pin_Max = Pin_Max(end);
    plot(Pin_dBm_vec+3, Pout_ADC_vec, '-o','LineWidth',1.2); hold on;
	plot(Pin_dBm_vec+3 , ones(size(Pin_dBm_vec))*14,'k--');
