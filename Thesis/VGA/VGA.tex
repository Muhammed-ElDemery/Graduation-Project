...
power inorder to get the required SNR and not to saturate the 
...
\begin{table}[htpb]
    ...
    \begin{tabular}{|c|c|}
        \hline
        \textbf{Paramter} & \textbf{Value} \\ \hline
...
Maximum gain affects the sensitivity of the reciever , VGA should
output suffecient voltage swing to ADC.
...
Non-linearty) and the other is INL (Internal Non-linearity),
the first describe how much the step varies from ideal required
step (in our case 1dB) and the latter is how much the step vaires
...
VGAs can fall into two main catagories one is Amplifier-Attenuator
and the other is Atteruator-Amplifier.\\
...
\begin{figure}[htbp]
  ...
  \caption{Resisitive feedback VGA}
...
is no single stage that can do both specailly for high gain 
...
Gain of 20dB and OIP3 = 15dBm, Gain variablity will be chosen
to maximize total OIP3 , this is achived later by varing the LNA 
...
base for the LNA architecture but achiving gain = 10 dB with 
...
on linearity , this can achives the required 15dBm OIP3.
...
resistive feedback Amplifier , so the ouptut stage of the LNA
better to be low impedance.\\
Lastly, adding gain variablity  to the LNA , current feedback 
...
\subsubsection{Resisitive feedback Opamp}%
...
\subsubsection{Resisitive and Capacitive DACs}%
\label{ssub:Resisitive and Capacitive DACs}
Resisitive and Capacitive DACs switches are thin oxid mos switches
...
VON =1.6V and VOFF=0V with nmos switchs (Figure~\ref{fig:VGA_DAC-eps})
\begin{figure}[htpb]
    \centering
    \includegraphics[width=0.8\textwidth]{VGA_DAC.eps}
    \caption{VGA DACs variablity}
...
$\leq 0.1dB$ which is expeceted from schematic results, BW apears
...
\begin{figure}[htbp]
  ...
  \caption{Small singal gain vs frequency across states}
...
in OIP3 is due to gain variablity since both LNA and resistive
feedback stage acts as Amplifier-Attenuator architecture (meaning)
constant IIP3 or more accurately Amplifier-Attenuator-Ampliefier
Architecure, here is a comparison in 
...
\begin{figure}[htbp]
  ...
  \caption{Dummy Amplifier-Attenuator-Ampliefier Vs real OIP3}
...
\section{Summary}%
The final VGA acts as Amplifier-Attenuator-Ampliefier architecture,
...
\begin{table}[htpb]
    ...
    \begin{tabular}{l c c c c}
        \textbf{Paramter} & \textbf{Specification}  &
