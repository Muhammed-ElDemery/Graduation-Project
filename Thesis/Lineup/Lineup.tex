...
Here is the system requirements, it's a Macro-cell base station 
receiver  with multi-standards , RF frequency Band from 400MHz 
to 8GHz  with RF Bandwidth of 400MHz , the receiver is a
direct conversion receiver with intended Baseband Bandwidth  of 
...
in-band blocker, our Analysis will focus on only one CW blocker
...
Gain = 22dB.\\
This means sensitivity of this receiver (Assuming minimum 
accepted SNR = 0dB).
\begin{equation}
...
large gain in the RF chain , so the remaining gain is added in
...
Since LNA gain is reduced in the RF chain a lot of losses can 
degrade receiver noise figure , so an RF Amplifier is added 
...
increased to 600MHz (due to DPD) it was hard to implement 
...
\subsection{Final Lineup specifications}%
...
for simulating SDR and non-linearities , power series (Memoryless-
...
This approach has some benefits like it takes the accounts for
not only IM3 but Gain compression and receiver desensitization
due to blockers , also it takes account for IM2s , so results 
from this simulation gives specification on small signal IP3 that
are approximately valid for large signal (ignoring $\alpha_5$).
...
picture of Non-linearities output spectrum of the receiver it 
...
is used (it appears like 3dB less).

\begin{table}[htpb]
    \centering
    \caption{Signal Parameter Table}
...
\begin{figure}[htbp]
  ...
  \caption{Output spectrum (Non-linearities) signal total
  power = -47dBm }
...
the output before ADC and the other after the mixer before LPF,
this power detector gives informations about the blocker existence.
...
\end{equation}
Where X is some value chosen arbitrary,when this condition
...
attenuate 3dB in order to overcome RF Amp compression (Figure~\ref{fig:DSA1_Gain_policy_under_blocking.png}),
