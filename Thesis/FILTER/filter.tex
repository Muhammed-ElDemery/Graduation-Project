...
main purpose is to reject the in-band blocker at 300MHz, a 
...
It's clear that the most important metric of a filter is 
...
phase response , meaning all the signal components has the 
...
compression is very critical in our system specially for this 
...
Filters can be categorized into two main categories: Passive filters
and Active filters.
...
low frequencies , but  have power consumption, the implemented 
...
\subsubsection{Active filters Implementations}%
There are mainly to approaches for implementing active filters:
using Bi-quad sections or LC-ladder simulation.\\
For the Bi-quad sections implementations are sallen-key ,
...
is based on gyrators that simulate that transfer characteristics of
inductors.

\subsubsection{OPamp-RC Vs GM-C filters}%
OPamp-RC technique is based on OPamps with RC feedback , this 
technique is usually linear (because of feedback) and its noise
...
(no stability consecrations) , but it has a poor linearity.
\cite{Saari2012_ch3}
...
and non-idealities of the actual implementation , the final 
...
After we chose filter approximation , a suitable implementation
technique is required ,as discussed in Literature since we are 
at baseband passive LC-ladder implementation  is not possible 
because of large inductor values 
Figure~\ref{fig:passive_LC_Ladder_filter} , instead active 
implementation is used Particularly GM-C filter , because 
...
\begin{figure}[htbp]
  ...
  \caption{Passive LC ladder implementation (Large inductors)}
...
GM-cell is specified using losses mapping technique described 
in \cite{Losses_mapping}, actually this technique benefit is 
very limited due to infinite quality factor of zeros, required
intrinsic gain of GM cell $\approx$ 200 , and integrated input-referred noise for the filter $\leq 99 \mu V$ RMS from 
equation~\ref{eq:noise_figure_input_noise}.

\subsection{GM-Cell Implementation}%
...
output stage is required but non-dominant poles degrade quality
of the integrator
...
differential swing smaller for the input pairs which make third 
...
degeneration  which degrades noise figure a little but improves 
linearity for input pairs described by equation~\ref{eq:HD3_res_degen} \cite{resistive_degeneration} , where $N = gm\ RS$
...
full band 114.5$\mu V$ , summary of AC results shown in 
...
For HB simulation two tones with 1mV peak amplitude is separated by
10MHz are used.
...
\begin{table}[htpb]
    ...
    \begin{tabular}{l c c}
        \toprule
    \textbf{Parameter} & \textbf{Specifications} & 
